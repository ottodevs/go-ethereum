...
func (s *BlockChainService) GetData(address common.Address, blockNr rpc.BlockNumber) (string, error) {
	if block := blockByNumber(s.bc, blockNr); block != nil {
		state, err := state.New(block.Root(), s.bc.chainDb)
		if err != nil {
			return "", err
...
func (s *BlockChainService) GetStorageAt(address common.Address, key string, blockNr rpc.BlockNumber) (string, error) {
	if block := blockByNumber(s.bc, blockNr); block != nil {
		state, err := state.New(block.Root(), s.bc.chainDb)
		if err != nil {
			return "", err
